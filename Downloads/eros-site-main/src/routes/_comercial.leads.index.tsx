          ...
          <Button 
            ...
            onClick={() => {
              const exportData = filtered.map(lead => ({
                ...
                'CNPJ': lead.cnpj || '—',
                'Contato Principal': lead.contato_principal || '—',
                'Cargo do Contato': lead.cargo_contato || '—',
                'Telefone/WhatsApp': lead.telefone || '—',
                'Email': lead.email || '—',
                'Localização': lead.cidade ? `${lead.cidade} ${lead.uf ? `- ${lead.uf}` : ''}` : '—',
                'Segmento': lead.segmento || '—',
                'Temperatura': lead.temperatura || '—',
                'Origem': lead.origem || '—',
                'Status': lead.status || '—',
                'Data de Cadastro': lead.created_at ? new Date(lead.created_at).toLocaleDateString('pt-BR') : '—'
              }));
              ...
              const colWidths = [35, 25, 20, 25, 20, 20, 30, 25, 20, 15, 15, 20, 20];
              worksheet['!cols'] = colWidths.map(w => ({ wch: w }));
              
              XLSX.writeFile(workbook, `Relatorio_Leads_${new Date().toLocaleDateString('pt-BR').replace(/\//g, '-')}.xlsx`);
            }}
          >
        ...
        <SheetContent className="w-[100vw] sm:max-w-[800px] overflow-y-auto border-l-0 sm:border-l shadow-2xl">
          ...
          <div className="py-2">
            <LeadForm 
              ...
              onDelete={leadEditando?.id ? () => handleDeleteLead(leadEditando.id as string) : undefined}
            />
          </div>
        </SheetContent>
      </Sheet>
