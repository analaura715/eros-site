...
export const Route = createFileRoute("/login")({
  head: () => ({ 
    meta: [
      { title: "Nexa CRM | Plataforma Comercial Premium" }, 
...
function LoginPage() {
  ...
  const [email, setEmail] = useState("Ana Laura");
    ...
    try {
      ...
      if (!loginEmail.includes('@')) {
        const { data: userData, error: userError } = await supabase
          .from('usuarios')
          .select('email')
          .ilike('nome', loginEmail) // Busca por nome ignorando maiúsculas/minúsculas
          .single();
    ...
    <div className="w-full min-h-screen flex font-sans bg-[#F7F8FA]">
      ...
      <div className="hidden lg:flex w-1/2 relative bg-gradient-to-br from-[#060D27] via-[#0D1B4C] via-60% to-[#043343] flex-col justify-between p-14 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-5 pt-2">
          <NexaLogo className="h-20 w-20 shrink-0 drop-shadow-[0_0_35px_rgba(0,181,226,0.6)]" />
          <div className="flex flex-col leading-none">
            <span className="text-6xl font-extrabold tracking-tight text-white drop-shadow-md">nexa</span>
        ...
        >
          ...
          <div className="flex flex-col mb-10">
            <div className="flex items-center gap-2.5 mb-6">
              ...
              <span className="text-2xl font-bold tracking-tight text-[#0a1128]">nexa</span>
            </div>
